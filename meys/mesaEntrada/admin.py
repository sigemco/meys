...
class DocumentoAdmin(admin.ModelAdmin):
    ...
    list_display = ('nro_documento','Tipo','asunto','origen','Nro_sistema','Fecha_registro')
    list_filter=['Tramite','Tipo']
    search_fields = ['nro_documento','Nro_sistema','asunto','Tipo__tipo']
    list_per_page = 25
    autocomplete_fields = ['Documento_relacionado', 'pase', 'entiende','participa','interviene','copia']
    date_hierarchy = 'Fecha_registro'
    actions = ['exportar_a_excel_recibidos', 'exportar_a_excel_emitidos', 'exportar_hoja_ruta']
    ...
    def exportar_a_excel_recibidos(self, request, queryset):
        ...
        cabezera=['Tipo','Nro documento','Asunto','Unidad de origen','Nro sistema','Fecha registro','Obs']
        ...
        filename = 'Documentos recibidos-{0}.xlsx'.format(datetime.now().strftime('%d-%m-%Y-%H-%M-%S'))
    ...
    def exportar_a_excel_emitidos(self, request, queryset):
        ...
        cabezera=['Tipo','Nro documento','Asunto','Destinatario','Nro sistema','Fecha registro','Obs']
        worksheet.write_row('A5', cabezera,bold)
        ...
        # Set up the Http response.
        filename = 'Documentos emitidos-{0}.xlsx'.format(datetime.now().strftime('%d-%m-%Y-%H-%M-%S'))
        response = HttpResponse(
            output,
            content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        )
    ...
    def exportar_hoja_ruta(self, request, queryset):
        ...
        filename = 'Hoja de ruta-{0}.pdf'.format(datetime.now().strftime('%d-%m-%Y-%H-%M-%S'))
